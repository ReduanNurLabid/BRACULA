...
try {
    ...
    if ($checkTableStmt->rowCount() === 0) {
        ...
        foreach ($requiredColumns as $column => $definition) {
            $checkColumnQuery = "SHOW COLUMNS FROM ride_requests LIKE '$column'";
            $checkColumnStmt = $conn->prepare($checkColumnQuery);
            $checkColumnStmt->execute();
            
            if ($checkColumnStmt->rowCount() === 0) {
                // Column doesn't exist, add it
                echo "Adding $column column to ride_requests table...<br>";
                $alterQuery = "ALTER TABLE ride_requests ADD COLUMN $column $definition";
