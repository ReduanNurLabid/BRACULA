...
async function loadEvents() {
    try {
        ...
        const response = await fetch(`${BASE_URL}/api/events.php?${params}`);
...
async function handleRegister(eventId) {
    try {
        ...
        const response = await fetch(`${BASE_URL}/api/event_registration.php`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function checkRegistrationStatus() {
    ...
    try {
        for (const event of STATE.events) {
            const response = await fetch(`${BASE_URL}/api/event_registration.php?event_id=${event.event_id}&user_id=${STATE.currentUser.user_id}`);
            const result = await response.json();
