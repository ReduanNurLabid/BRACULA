...
function viewDetails(accommodationId) {
    ...
    if (accommodation.images && accommodation.images.length > 0) {
        accommodation.images.forEach(imageUrl => {
            ...
            imgContainer.innerHTML = `<img src="${imageUrl}" alt="${accommodation.title}" onerror="this.src='images/placeholder.jpg'">`;
            detailsGallery.appendChild(imgContainer);
        });
    } else {
        detailsGallery.innerHTML = '<div class="gallery-image"><img src="images/placeholder.jpg" alt="No Image Available"></div>';
    }
    
    // Store selected accommodation ID for contact form
    document.getElementById('send-inquiry-btn').dataset.accommodationId = accommodationId;
